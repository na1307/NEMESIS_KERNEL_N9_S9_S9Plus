...
FUNC_BUILD_DTIMAGE_TARGET()
{
	...
	crownlte)
		case $VARIANT in
		can|duos|eur|xx)
			DTSFILES="exynos9810-crownlte_eur_open_17 exynos9810-crownlte_eur_open_18
					exynos9810-crownlte_eur_open_19 exynos9810-crownlte_eur_open_21
					exynos9810-crownlte_eur_open_23 exynos9810-crownlte_eur_open_24
					exynos9810-crownlte_eur_open_25 exynos9810-crownlte_eur_open_26"
			;;
		*)
			echo "Unknown variant: $VARIANT"
			exit 1
...
FUNC_BUILD_RAMDISK()
{
	...
	crownlte)
		case $VARIANT in
		can|duos|eur|xx)
			rm -f $RDIR/ramdisk/SM-N960F/split_img/boot.img-zImage
			rm -f $RDIR/ramdisk/SM-N960F/split_img/boot.img-dtb
			mv -f $RDIR/arch/$ARCH/boot/boot.img-zImage $RDIR/ramdisk/SM-N960F/split_img/boot.img-zImage
			mv -f $RDIR/arch/$ARCH/boot/boot.img-dtb $RDIR/ramdisk/SM-N960F/split_img/boot.img-dtb
			cd $RDIR/ramdisk/SM-N960F
			./repackimg.sh --nosudo
			echo SEANDROIDENFORCE >> image-new.img
			;;
		*)
...
FUNC_BUILD_ZIP()
{
	...
	crownlte)
		case $VARIANT in
		can|duos|eur|xx)
			mv -f $RDIR/ramdisk/SM-N960F/image-new.img $RDIR/build/$MODEL-$VARIANT.img
			;;
		*)
			echo "Unknown variant: $VARIANT"
			exit 1
